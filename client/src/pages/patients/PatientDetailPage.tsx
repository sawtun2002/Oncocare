...
import { getPatient, updatePatient, type PatientInput } from "../../api/patients";
...
export function PatientDetailPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: (input: Partial<PatientInput>) => updatePatient(patientId, input),
    onSuccess: () => {
  ...
  const rescheduleMutation = useMutation({
    mutationFn: ({ id: apptId, ...input }: { id: number; doctorId: number; scheduledAt: string }) =>
      updateAppointment(apptId, input),
    onSuccess: invalidateAppointments,
  });

  const cancelMutation = useMutation({
    mutationFn: (apptId: number) => updateAppointmentStatus(apptId, "CANCELLED"),
    onSuccess: invalidateAppointments,
  });
  ...
  const doctorName = (doctorId: number) => doctorsQuery.data?.find((d) => d.id === doctorId)?.name ?? `#${doctorId}`;
