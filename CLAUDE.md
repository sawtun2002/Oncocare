...
- The header notice bell (`components/NoticeBell.jsx`) is **derived, not stored**: `lib/notices.js` turns the `["appointments"]` query the app already holds into a per-viewer list (an `AppointmentEvent` on an appointment you're a party to that you didn't cause — audience rules per role, mirrored in `API_CONTRACT.md`'s Notices table). The only persisted state is `User.notificationsReadAt`, stamped by `POST /api/auth/me/notifications/read` (`markNotificationsRead`) when the panel is opened; the unread badge is "events after that timestamp".
...
Shared entity shapes live in `client/src/types/index.js` (`User`, `Patient`, `Appointment`, `AppointmentEvent`, `Invoice`, `InvoiceItem`, and their status/role unions) as JSDoc `@typedef` blocks. Nothing enforces them at build time — they exist so editors autocomplete and so `/contract-check`, the `contract-auditor` agent and the Java hand-off have a machine-readable target. They mirror the entities in `API_CONTRACT.md` field-for-field: if one changes, change the other in the same edit.
